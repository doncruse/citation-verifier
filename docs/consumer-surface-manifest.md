...
|---|---|---|---|
| `verify_batch()` doesn't pop `needs_caption_investigation` / `_cluster_for_investigation` before calling `_finalize_result` | `src/citation_verifier/verifier.py:2890` | Verifier-internal bug; Phase 5 froze the schema and consumers, not verifier internals. Caught during Phase 5 Task 2 (the route was switched to per-citation `verify_async` as a workaround). | `verify_batch()` raises TypeError on any name-mismatch hit. Affects the `audit-misses` CLI and any future caller that uses `verify_batch()` directly. The web app's `/api/qc/run-batch` is unaffected after Task 2 (uses per-citation `verify_async`). |
| `verify_batch()` doesn't accept a pre-constructed `AsyncCourtListenerClient` | `src/citation_verifier/verifier.py:2825-2871` | Plan's contingency path. Without client injection, BYOK tokens from web routes can't reach `verify_batch()`; web `/api/verify` falls back to per-citation `verify_async` for BYOK preservation. | API-call savings lost for web batches (one citation_lookup per cite instead of one shared call). Fix: add `client=` or `api_token=` parameter to `verify_batch`. |
