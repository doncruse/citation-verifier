...
| **Re-run gold-pair Opus at full text** — replaces Task 10's mis-labeled rows | Task 10's 117 gold-pair verdicts are stored as `opinion_window_chars=60000` but were scored at 20K. Cheap re-run (~127 Opus calls, all subscription) gives the canonical full-text baseline. | Honest calibration column for v2 to extend; supersedes mis-labeled data |
