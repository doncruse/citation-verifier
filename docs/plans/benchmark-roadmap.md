...
| Larger N (500–1000) | Originally predicated on the assessor calibration (v1.1, done) cutting per-cell cost; that path is closed. Larger N now needs either a bigger Opus budget, prompt-tuned cheaper assessors (separate study), or a different scoring rubric |
