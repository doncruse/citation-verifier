...
| `tools/ab_test_runner.py` | A/B harness (§9): runs the assess verb over copies of the frozen corpora with a named config (`tests/ab_test_configs.json`), scores via `scoring.score_workdir`; `--replay` scores the cassettes offline; the prompt comes from the byte-pinned template, not a local copy. `tests/ab_test_cases.json` stays the human-review ledger; `include_hints` configs require `prompt_version: assess-v2` (v1 is byte-pinned, no hint) and run a Haiku prescreen phase into the copy's claims before assess — configs `opus-v2` / `opus-v2-hints` are the §6.7 A/B arms (run 2026-06-13: hints hurt → prescreen default OFF) |
