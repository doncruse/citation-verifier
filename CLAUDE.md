...
| `tools/ab_test_runner.py` | A/B harness (§9): runs the assess verb over copies of the frozen corpora with a named config (`tests/ab_test_configs.json`), scores via `scoring.score_workdir`; `--replay` scores the cassettes offline; the prompt comes from the byte-pinned template, not a local copy. `tests/ab_test_cases.json` stays the human-review ledger; `include_hints` configs are rejected until assess-v2 |
